...
class Tarea:
    ...
    def obtener_informacion(self):
        info = f"Descripcion: {self.descripcion}, Completada: {self.completada}"
        if self.fecha_vencimiento:
            info += f", Fecha de vencimiento: {self.fecha_vencimiento}"
        if self.recordatorio:
            info += f", Recordatorio: {self.recordatorio}"
        return info
...
# Ejemplos de uso 
tarea1 = Tarea("Hacer la compra", fecha_vencimiento=datetime.now()+ timedelta(days=2), recordatorio="No olvidar la lista")
tarea2 = Tarea("Estudiar para el examen", fecha_vencimiento=datetime.now() + timedelta(weeks=1))
tarea3 = Tarea("Ir al gimnasio", recordatorio="No olvidar la botella de agua")

# Agregar a la lista y mostrarlas
lista_tareas = [tarea1, tarea2, tarea]