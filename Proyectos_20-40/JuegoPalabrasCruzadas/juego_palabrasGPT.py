...
def generar_tablero(palabras):
    ...
    for palabra in palabras:
        # Elige aleatoriamente la dirección de la palabra (horizontal o vertical)
        direccion = random.choice(['horizontal', 'vertical'])
        x = random.randint(0, 9)
        y = random.randint(0, 9)

        # Busca una posición válida para la palabra en el tablero
...
def es_posicion_valida(tablero, palabra, x, y, direccion):
    # Verifica si la posición y dirección elegidas son válidas para colocar la palabra
    if direccion == 'horizontal' and y + len(palabra) <= 10:
        for i in range(len(palabra)):
            # Verifica si hay alguna colisión con otra palabra ya colocada
...
def colocar_palabra(tablero, palabra, x, y, direccion):
    # Coloca la palabra en el tablero en la posición y dirección especificadas
...
def main():
    # Lista de palabras para las palabras cruzadas
    palabras = ['python', 'java', 'ruby', 'javascript', 'html']
    
    # Genera el tablero y lo muestra
    tablero = generar_tablero(palabras)
